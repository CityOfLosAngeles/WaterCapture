...
for(i in 2:ncol(ducommun)){
  formatted[[i-1]] <- ducommun[[i]] %>% str_remove_all(., "\\s") %>% as.numeric()
}
...
for(i in 2:ncol(USC)){
  formatted[[i-1]] <- USC[[i]] %>% str_remove_all(., "\\s") %>% as.numeric()
}
...
for(i in 2:ncol(schoolhouse)){
  formatted[[i-1]] <- schoolhouse[[i]] %>% str_remove_all(., "\\s") %>% as.numeric()
}
...
for(i in 2:ncol(hollywood)){
  formatted[[i-1]] <- hollywood[[i]] %>% str_remove_all(., "\\s") %>% as.numeric()
}
...
new_row$rain_in <- total_rainfall_LA
